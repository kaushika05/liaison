...
const modelAgentDecisionSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    closingText: z.string().max(400).nullable(),
...
function normalizeDecision(value: z.infer<typeof modelAgentDecisionSchema>): AgentDecision {
  ...
  if (value.action === "END_CALL")
    return agentDecisionSchema.parse({ ...value, closingText: value.closingText ?? undefined });
  return agentDecisionSchema.parse(value);
}

export interface ModelUsageRecord {
  operation: "planner" | "controller" | "outcome" | "messaging_intent";
...
export class ModelService {
  ...
  constructor(
    ...
    private readonly onUsage?: (item: ModelUsageRecord) => void,
  ) {
    this.client =
  ...
  drainUsageRecords(): ModelUsageRecord[] {
    return this.usageRecords.splice(0);
  }
  private capture(
  ...
  async plan(id: string, phone: string, intake: CaseIntake): Promise<CallBrief> {
    if (!this.client) {
      ...
      const known = lines(intake.knownFactsText);
      const chronology = lines(intake.chronologyText).map((event) => ({ id: randomUUID(), event }));
      return callBriefSchema.parse({
        id,
        version: 1,
        companyName: intake.companyName,
        phoneNumberE164: phone,
        userFirstName: intake.userFirstName,
        title: `${intake.companyName} support request`,
        issueSummary: intake.issueDescription,
        chronology,
        desiredOutcome: intake.desiredOutcome,
        acceptableAlternatives: lines(intake.acceptableAlternativesText),
        unacceptableOutcomes: lines(intake.unacceptableOutcomesText),
        knownFacts: known,
        ...
        strategySteps: [
          "Navigate the support menu without disclosing case details.",
          "Disclose the automated accessibility assistant to a human representative and ask consent.",
          "Explain the issue briefly using only supplied facts.",
          "Ask for the requested outcome.",
          "Confirm any concrete action, case number, and timeline.",
          "Summarize unresolved items before ending.",
        ],
        likelyApprovalPoints: [
          "Any personal-data disclosure",
          "Any fee, account change, cancellation, schedule, or fallback outcome",
        ],
        warnings: risks.map(
          (risk) =>
            `Potential prohibited high-risk category detected: ${risk}. Real calling is blocked until the case is edited.`,
        ),
        authority: intake.authority ?? defaultAuthority,
      });
    }
    const safeInput = {
      ...intake,
      disclosures: intake.disclosures.map((card) => ({
        label: card.label,
        category: card.category,
        permission: card.permission,
        allowedChannels: card.allowedChannels,
    ...
    const response = await this.client.responses.parse({
      ...
      input: [
        ...
        { role: "user", content: JSON.stringify({ id, phoneNumberE164: phone, intake: safeInput }) },
      ],
      text: { format: zodTextFormat(modelCallBriefSchema, "call_brief") },
    });
  ...
  async decide(context: unknown): Promise<AgentDecision> {
    ...
    const response = await this.client.responses.parse({
      model: this.config.CONTROLLER_MODEL,
      reasoning: { effort: this.config.OPENAI_REASONING_EFFORT },
      input: [
        { role: "system", content: controllerPrompt },
        { role: "user", content: `<untrusted_call_context>${JSON.stringify(context)}</untrusted_call_context>` },
      ],
      text: { format: zodTextFormat(modelAgentDecisionSchema, "agent_decision") },
    });
  ...
  async outcome(input: {
    ...
    deterministicFallback: OutcomeReport;
  }): Promise<OutcomeReport> {
    if (!this.client) return input.deterministicFallback;
    const response = await this.client.responses.parse({
      model: this.config.OUTCOME_MODEL,
      reasoning: { effort: this.config.OPENAI_REASONING_EFFORT },
      input: [
        { role: "system", content: outcomePrompt },
        { role: "user", content: JSON.stringify({ brief: input.brief, transcript: input.transcript }) },
      ],
      text: { format: zodTextFormat(modelOutcomeReportSchema, "outcome_report") },
    });
    ...
    return outcomeReportSchema.parse({
      ...response.output_parsed,
      llmUsage: response.output_parsed.llmUsage ?? undefined,
    });
  }

  async classifyMessagingIntent(input: {
    threadState: SupportThreadState;
    message: string;
  }): Promise<MessagingIntentClassification> {
    if (!this.client) {
      return messagingIntentClassificationSchema.parse({
        intent:
          input.threadState === "IDLE" ||
          input.threadState === "COLLECTING_ISSUE" ||
          input.threadState === "AWAITING_INFORMATION"
            ? "ADD_CONTEXT"
            : input.threadState === "CALL_ACTIVE" || input.threadState === "AWAITING_USER_DECISION"
              ? "PRIVATE_CALL_INSTRUCTION"
              : "UNCLEAR",
        companyName: null,
        phoneNumber: null,
        desiredOutcome: null,
        contextToAdd: input.message,
    ...
    const response = await this.client.responses.parse({
      ...
      input: [
        ...
        {
          role: "user",
          content: `<thread_state>${input.threadState}</thread_state>\n<untrusted_user_message>${input.message}</untrusted_user_message>`,
        },
      ],
      text: { format: zodTextFormat(messagingIntentClassificationSchema, "messaging_intent") },
    });
    this.capture("messaging_intent", response);
    if (!response.output_parsed) throw new Error("MODEL_MESSAGING_INTENT_EMPTY");
    return messagingIntentClassificationSchema.parse(response.output_parsed);
  }
}
