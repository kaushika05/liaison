 ...
 */
export function semanticEventsFromCapturedFacts(input: unknown, transcript: TranscriptTurn[]): SemanticProjection[] {
  ...
  for (const fact of facts) {
    ...
    if (fact.kind === "DEPARTMENT") {
      if (!deterministicKinds.has("DEPARTMENT_REACHED")) {
        results.push({
          event: { kind: "DEPARTMENT_REACHED", department: quote },
          message: `The call reached ${quote}.`,
        });
      }
      continue;
    }
    if (fact.kind === "CASE_NUMBER") {
      if (!deterministicKinds.has("CASE_NUMBER_RECEIVED")) {
        results.push({
          event: { kind: "CASE_NUMBER_RECEIVED", evidence: evidence(turn, quote) },
          message: `A case or confirmation number was received: ${quote}.`,
        });
      }
      continue;
    }
    if (fact.kind === "DEADLINE") {
      if (!deterministicKinds.has("DEADLINE_RECEIVED")) {
        results.push({
          event: { kind: "DEADLINE_RECEIVED", evidence: evidence(turn, quote) },
          message: `A concrete timeline was received: ${quote}.`,
        });
      }
      continue;
    }
    if (fact.kind === "COMMITMENT") {
      results.push({
        event: { kind: "FACT_CONFIRMED", fact: quote, evidence: evidence(turn, quote) },
        message: `A transcript-grounded commitment statement was captured: ${quote}.`,
      });
      continue;
    }
    if (fact.kind === "REPRESENTATIVE_NAME") {
      results.push({
        event: { kind: "FACT_CONFIRMED", fact: quote, evidence: evidence(turn, quote) },
        message: `The representative identified themself as ${quote}.`,
      });
      continue;
    }
    results.push({
      event: { kind: "FACT_CONFIRMED", fact: quote, evidence: evidence(turn, quote) },
      message: `A transcript-grounded fact was captured: ${quote}.`,
    });
  }
  return results;
}
...
export function semanticEventsFromFinalTurn(turn: TranscriptTurn): SemanticProjection[] {
  ...
  const results: SemanticProjection[] = [];
  const department =
    /\b(?:you(?:'ve| have) reached|this is [^.!?]{1,60}? in)\s+([a-z][a-z &-]{1,50}?)(?:\s+(?:department|support|customer care))?(?:[.!?,]|\s+how\b)/i.exec(
      text,
  ...
  const negatedTransfer =
    /\b(?:cannot|can't|can not|unable to|won't|will not|do not|don't|no)\b[^.!?]{0,48}\btransfer/i.test(text);
  if (/\b(?:transfer|transferring)\b/i.test(text) && !negatedTransfer) {
    ...
    results.push({
      event: { kind: "TRANSFER_STARTED", ...(destination ? { destination } : {}) },
      message: destination ? `A transfer started to ${destination}.` : "A call transfer started.",
    });
  }
  if (
    /\b(?:authenticate|authentication|verify|verification|account number|order number|billing (?:zip|address)|date of birth)\b/i.test(
      text,
    )
  ) {
    const category = /billing zip/i.test(text)
      ? "billing ZIP"
      : /date of birth/i.test(text)
        ? "date of birth"
        : /account number/i.test(text)
          ? "account number"
          : /order number/i.test(text)
            ? "order number"
            : "account verification";
    results.push({
      event: { kind: "AUTHENTICATION_REQUESTED", category },
      message: `The representative requested ${category}; Liaison will not send sensitive values through SMS.`,
    });
  }
  const negatedOffer =
    /\b(?:no|not|cannot|can't|can not|unable to|won't|will not|do not|don't|decline|refuse)\b[^.!?]{0,64}\b(?:offer|refund|credit|discount|upgrade|downgrade)\b/i.test(
      text,
    );
  if (
    /\b(?:offer|instead|would you accept|refund|credit|discount|upgrade|downgrade)\b/i.test(text) &&
    !/\b(?:approved|applied|completed|confirmed)\b/i.test(text) &&
    !negatedOffer
  ) {
    const description = safeOfferDescription(text);
    const amountCents = money(text);
    results.push({
      event: { kind: "OFFER_MADE", description, ...(amountCents === undefined ? {} : { amountCents }) },
      message: `${description}${amountCents === undefined ? "" : ` Amount: $${(amountCents / 100).toFixed(2)}.`}`,
    });
  }
  ...
  if (caseMatch) {
    ...
    results.push({
      event: { kind: "CASE_NUMBER_RECEIVED", evidence: evidence(turn, quote) },
      message: `A case or confirmation number was received: ${caseMatch[1]}.`,
    });
  }
  const deadlineMatch =
    /\b(?:within\s+\w+(?:\s+\w+){0,3}|by\s+(?:today|tomorrow|[A-Z][a-z]+(?:\s+\d{1,2})?)|effective\s+(?:today|tomorrow)|next\s+billing\s+statement|\d+\s+(?:business\s+)?days?)\b/i.exec(
      text,
    );
  if (deadlineMatch) {
    results.push({
      event: { kind: "DEADLINE_RECEIVED", evidence: evidence(turn, deadlineMatch[0]) },
      message: `A concrete timeline was received: ${deadlineMatch[0]}.`,
    });
  }
  return results;
}
...
export function semanticEventsFromOutcome(report: OutcomeReport): SemanticProjection[] {
  ...
  if (report.department)
    results.push({
      event: { kind: "DEPARTMENT_REACHED", department: report.department.value },
      message: `Verified department: ${report.department.value}.`,
    });
  if (report.caseNumber)
    results.push({
      event: { kind: "CASE_NUMBER_RECEIVED", evidence: report.caseNumber.evidence },
      message: `Verified case number: ${report.caseNumber.value}.`,
    });
  for (const deadline of report.deadlines)
    results.push({
      event: { kind: "DEADLINE_RECEIVED", evidence: deadline.evidence },
      message: `Verified timeline: ${deadline.value}.`,
    });
  if (report.status === "RESOLVED" && report.resolution)
    results.push({
      event: { kind: "RESOLUTION_VERIFIED" },
      message: "The transcript-grounded resolution was verified.",
    });
  if (report.status === "DISCONNECTED")
    results.push({ event: { kind: "CALL_DISCONNECTED" }, message: "The telephone call disconnected unexpectedly." });
  return results;
}
