export type MessagingCommand =
  | { kind:"START" | "STOP" | "HELP" | "NEW" | "STATUS" | "LINK" | "EDIT" | "CANCEL" | "PAUSE" | "RESUME" | "APPROVE_PLAN" }
...
const simple = new Set(["START","STOP","HELP","NEW","STATUS","LINK","EDIT","CANCEL","PAUSE","RESUME"] as const);
...
export function parseMessagingCommand(input:string):MessagingCommand|null {
  ...
  if(simple.has(upper as typeof simple extends Set<infer T> ? T : never)) return {kind:upper as Extract<MessagingCommand,{kind:string}>["kind"]} as MessagingCommand;
  ...
  const call=/^CALL\s+([A-Z0-9]{4,8})$/i.exec(text); if(call) return {kind:"CALL",code:call[1].toUpperCase()};
  const mode=/^MODE\s+(ASSIST|COPILOT|DELEGATE)$/i.exec(text); if(mode) return {kind:"MODE",mode:mode[1].toUpperCase() as "ASSIST"|"COPILOT"|"DELEGATE"};
  const speech=/^SAY:\s*(.+)$/is.exec(text); if(speech) return {kind:"EXACT_SPEECH",text:speech[1].trim()};
  return null;
}
