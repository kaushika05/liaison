...
export type EndReason = "RESOLVED"|"PARTIALLY_RESOLVED"|"UNRESOLVED"|"REPRESENTATIVE_REFUSED_AUTOMATION"|"AUTHENTICATION_REQUIRED"|"USER_REQUESTED"|"TECHNICAL_FAILURE"|"POLICY_BLOCKED";
...
export class SimulatedTelephonyAdapter implements TelephonyAdapter {
  ...
  async startCall(input:StartCallInput):Promise<StartCallResult>{ this.actions.push({callId:input.callId,type:"start",value:input.destination}); return {providerCallId:`SIM-${input.callId}`}; }
  async speak(callId:string,text:string):Promise<void>{ this.actions.push({callId,type:"speak",value:text}); }
  async sendDigits(callId:string,digits:string):Promise<void>{ this.actions.push({callId,type:"digits",value:digits}); }
  async pauseAgent(callId:string):Promise<void>{ this.actions.push({callId,type:"pause"}); }
  async resumeAgent(callId:string):Promise<void>{ this.actions.push({callId,type:"resume"}); }
  async endCall(callId:string,reason:EndReason):Promise<void>{ this.actions.push({callId,type:"end",value:reason}); }
}
...
export class TwilioConversationRelayAdapter implements TelephonyAdapter {
  ...
  private send(callId:string,payload:object):void{ const socket=this.sockets.get(callId); if(!socket || socket.readyState!==socket.OPEN) throw new Error("TWILIO_RELAY_NOT_CONNECTED"); socket.send(JSON.stringify(payload)); }
  async speak(callId:string,text:string,options?:SpeakOptions):Promise<void>{ this.send(callId,{type:"text",token:text,last:true,interruptible:options?.interruptible??true,preemptible:false}); }
  async sendDigits(callId:string,digits:string):Promise<void>{ this.send(callId,{type:"sendDigits",digits}); }
  async pauseAgent(callId:string):Promise<void>{ void callId; /* Application-level pause; transcription remains connected. */ }
  async resumeAgent(callId:string):Promise<void>{ void callId; /* Application-level resume. */ }
  async endCall(callId:string,reason:EndReason):Promise<void>{
    const socket=this.sockets.get(callId); if(socket && socket.readyState===socket.OPEN) socket.send(JSON.stringify({type:"end",handoffData:JSON.stringify({reasonCode:reason})}));
    const sid=this.providerIds.get(callId); if(sid) await this.client.calls(sid).update({status:"completed"});
    this.sockets.delete(callId); this.providerIds.delete(callId);
  }
  async endOrphanedCall(providerCallId:string):Promise<void>{ await this.client.calls(providerCallId).update({status:"completed"}); }
}
