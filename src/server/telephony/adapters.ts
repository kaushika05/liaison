...
export class TwilioConversationRelayAdapter implements TelephonyAdapter {
   ...
   */
  bindProviderCallId(callId:string,providerCallId:string):void{ this.providerIds.set(callId,providerCallId); }
  ...
  async endCall(callId:string,reason:EndReason):Promise<void>{
    const socket=this.sockets.get(callId); const relayOpen=Boolean(socket && socket.readyState===socket.OPEN);
