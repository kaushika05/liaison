...
export interface MessagingThreadSummary {
  ...
  messagingOptState: "UNKNOWN" | "OPTED_IN" | "OPTED_OUT";
}
export interface MessagingThreadMessage {
  id: string;
  direction: "INBOUND" | "OUTBOUND";
  providerKind: "WEB" | "TWILIO_SMS" | "SIMULATOR";
  redactedBody: string;
  createdAt: string;
...
export interface MessagingAttentionSummary {
  ...
  status: string;
  question: string;
  choices: Array<{ id: string; shortCode: string; label: string; effect: string }>;
  expiresAt: string;
  secureActionRequired: boolean;
}
export interface MessagingCommitmentSummary {
  id: string;
  party: "COMPANY" | "USER" | "AGENT" | "UNKNOWN";
  status: string;
  description: string;
...
export interface MessagingThreadSnapshot {
  ...
  attention: MessagingAttentionSummary | null;
  commitments: MessagingCommitmentSummary[];
...
export interface SecureActionDetail {
  tokenState: "VALID";
  actionType: string;
  attention: MessagingAttentionSummary;
  case: CaseDetail | null;
  call: CallSnapshot | null;
