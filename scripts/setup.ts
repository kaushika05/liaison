...
const guardedOutput = new Writable({
  write(chunk: string | Uint8Array, _encoding, done) {
    if (!suppressTerminalEcho) output.write(chunk);
    done();
  },
});
...
const ask = async (question: string, fallback = "") =>
  interactive ? (await rl.question(`${question}${fallback ? ` [${fallback}]` : ""}: `)).trim() || fallback : fallback;
...
const confirm = async (question: string, defaultNo = true) => {
  if (!interactive) return !defaultNo;
  const answer = (await rl.question(`${question} ${defaultNo ? "[y/N]" : "[Y/n]"} `)).trim().toLowerCase();
  return defaultNo ? answer === "y" || answer === "yes" : answer !== "n" && answer !== "no";
};

try {
  if (existsSync(target) && !dryRun) {
    if (!(await confirm(`${target} already exists. Overwrite it?`)))
      throw new Error("Setup stopped without changing the existing file.");
  }
  ...
  if (ownerPhone && !/^\+[1-9]\d{7,14}$/.test(ownerPhone))
    throw new Error("Owner phone must use E.164 format, for example +13045550123.");
  ...
  const authToken =
    messaging === "twilio_sms" || voice === "twilio" ? await askSecret("Twilio Auth Token (hidden)") : "";
  const messagingSid =
    messaging === "twilio_sms" ? await ask("Twilio Messaging Service SID (blank to use an SMS number)", "") : "";
  ...
  for (const [name, value] of [
    ["SMS sender", smsFrom],
    ["voice sender", voiceFrom],
  ] as const)
    if (value && !/^\+[1-9]\d{7,14}$/.test(value))
      throw new Error(`${name} must use E.164 format, for example +13045550123.`);
  ...
  const values = [
    ...
    "",
    "# Messaging",
    `MESSAGING_MODE=${messaging}`,
    "ALLOW_REAL_MESSAGING=false",
    `TWILIO_MESSAGING_SERVICE_SID=${messagingSid}`,
    `TWILIO_SMS_FROM_NUMBER=${smsFrom}`,
    "MESSAGING_REGISTRATION_CONFIRMED=false",
    "SMS_UPDATE_DETAIL=STANDARD",
    "SMS_DECISION_TIMEOUT_SECONDS=90",
    "MAX_USER_WAIT_SECONDS=180",
    "SMS_MAX_SEGMENTS_PER_MESSAGE=3",
    "ESTIMATED_SMS_COST_PER_SEGMENT_USD=0",
    "",
    "# Voice and shared Twilio credentials",
    `TELEPHONY_MODE=${voice}`,
    "ALLOW_REAL_CALLS=false",
    `TWILIO_ACCOUNT_SID=${accountSid}`,
    `TWILIO_AUTH_TOKEN=${authToken}`,
    `TWILIO_VOICE_FROM_NUMBER=${voiceFrom}`,
    "TWILIO_FROM_NUMBER=",
    "MAX_CALL_DURATION_MINUTES=30",
    "MAX_CALLS_PER_DAY=5",
    "ALLOWED_DESTINATION_PREFIXES=+1",
    "ESTIMATED_TELEPHONY_COST_PER_MINUTE_USD=0.084",
    "",
    "# Security, retention, and logging",
    "SECURE_ACTION_LINK_TTL_MINUTES=10",
  ...
  output.write(
    dryRun
      ? "Access key: generated only when the configuration is written.\n"
  ...
  output.write(
    "Real SMS and calls remain disabled until the operator explicitly enables them; provider use may incur charges.\n",
  );
} finally {
  rl.close();
}
