...
const issue = "Call Xfinity at +18009346489. They charged me a $35 installation fee even though installation was promised free. I want the fee removed. Account credit is acceptable only if it is at least $35. Do not change my plan or accept any new charge.";
...
async function waitFor(
  ...
  const snapshot = context.messaging.snapshot();
  throw new Error(`DEMO_TIMEOUT:${description}:thread=${snapshot.thread.state}:call=${snapshot.call?.state ?? "none"}:paused=${snapshot.call?.paused ?? false}:attention=${snapshot.attention?.tier ?? "none"}`);
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (snapshot.thread.state !== "AWAITING_PLAN_APPROVAL" || !snapshot.case?.brief) throw new Error("DEMO_PLAN_NOT_CREATED");
    ...
    const authorizationMessage = [...snapshot.messages].reverse().find((message) => message.direction === "OUTBOUND" && /CALL [23456789A-HJ-NP-Z]{6}/.test(message.redactedBody));
    ...
    snapshot = await waitFor(context, "low-consequence decision", (value) => value.attention?.tier === "LOW_CONSEQUENCE");
    ...
    if (snapshot.call?.paused) {
      const latest = [...snapshot.messages].reverse().find((message) => message.direction === "OUTBOUND")?.redactedBody.split("\n")[0] ?? "no-reply";
    ...
    const materialMessage = [...snapshot.messages].reverse().find((message) => message.direction === "OUTBOUND" && message.redactedBody.includes("/a/"));
    ...
    await context.messaging.resolveSecureAction(materialToken, { decision: "APPROVE" }).then(
      ...
      (error: unknown) => { if (!(error instanceof Error) || !error.message.includes("MATERIAL_CONFIRMATION_REQUIRED")) throw error; },
    );
    await context.messaging.resolveSecureAction(materialToken, { decision: "APPROVE", confirmation: "CONFIRM" }).then(
      ...
      (error: unknown) => { if (!(error instanceof Error) || !error.message.includes("CONDITIONAL_AUTHORITY_DENIED")) throw error; },
    );
    await context.messaging.resolveSecureAction(materialToken, { decision: "REJECT" });
    snapshot = await waitFor(context, "grounded outcome", (value) => value.thread.state === "COMPLETED" && value.commitments.length > 0);

    const callRow = context.database.db.prepare("SELECT id FROM calls ORDER BY started_at DESC LIMIT 1").get() as { id: string } | undefined;
    ...
    await context.messaging.receiveWebText(`CALL ${code}`);
    const callCount = (context.database.db.prepare("SELECT COUNT(*) AS count FROM calls").get() as { count: number }).count;
    if (callCount !== 1) throw new Error("DEMO_CALL_CODE_REPLAYED");

    process.stdout.write(`PASS: ${snapshot.case?.companyName} case completed with ${snapshot.commitments.length} evidence-bound commitment(s), one call, and no credential disclosure.\n`);
  } finally {
