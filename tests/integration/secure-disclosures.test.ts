...
describe("authenticated secure disclosure attachment", () => {
  it("keeps plaintext only in volatile memory and returns and persists metadata only", async () => {
    ...
    const fetched = await context.app.inject({ method: "GET", url: `/api/cases/${item.id}`, headers: { cookie: cookie! } });
    expect(fetched.statusCode).toBe(200);
    expect(fetched.body).not.toContain(payload.value);
    const persisted = context.database.db.prepare("SELECT intake_json,disclosure_metadata_json FROM cases WHERE id=?").get(item.id) as { intake_json: string; disclosure_metadata_json: string };
    const auditPayloads = context.database.db.prepare("SELECT payload_json FROM events WHERE case_id=?").all(item.id) as Array<{ payload_json: string }>;
    expect(JSON.stringify({ persisted, auditPayloads })).not.toContain(payload.value);
    expect(context.disclosures.resolve(item.id,updated.disclosures[0].id,"SPEECH","account identification for authentication")?.value).toBe(payload.value);
  });
  ...
  it("rejects prohibited secrets and delivery channels that cannot safely carry the value", async () => {
    ...
    const invalidDtmf = await context.app.inject({ method: "POST", url, headers: { cookie: cookie! }, payload: { category: "ORDER_NUMBER", value: "ORDER-77", allowedChannels: ["DTMF"] } });
    expect(invalidDtmf.statusCode).toBe(400);
    expect(invalidDtmf.body).not.toContain("ORDER-77");
    const addressDtmf = await context.app.inject({ method: "POST", url, headers: { cookie: cookie! }, payload: { category: "ADDRESS", value: "15 Main Street", allowedChannels: ["DTMF"] } });
    expect(addressDtmf.statusCode).toBe(400);
    expect(addressDtmf.body).not.toContain("15 Main Street");
    const prohibited = await context.app.inject({ method: "POST", url, headers: { cookie: cookie! }, payload: { category: "ACCOUNT_NUMBER", value: "password: hunter2", allowedChannels: ["SPEECH"] } });
