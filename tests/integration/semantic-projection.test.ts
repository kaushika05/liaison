...
async function prepareCall(app: AppContext, callId: string): Promise<{ caseId: string; threadId: string; turn: TranscriptTurn }> {
...
describe("runtime semantic projection", () => {
  it("grounds structured facts and atomically records confirmed-commitment events", async () => {
    ...
    await waitUntil(() => app.database.listSemanticCallEvents(callId).some((event) => event.eventType === "FACT_CONFIRMED"));
    const captured = app.database.listSemanticCallEvents(callId).filter((event) => event.eventType === "FACT_CONFIRMED");
    ...
    const outcomeEvent = { sequence: 11, callId, caseId, type: "OUTCOME_GENERATED", data: { grounded: true }, timestamp: report.endedAt } as const;
    ...
    expect(app.database.listSemanticCallEvents(callId).filter((event) => event.eventType === "COMMITMENT_CONFIRMED")).toHaveLength(2);
  ...
  it("deduplicates a disconnect semantic event from call end and outcome", async () => {
    ...
    app.service.events.emit("call:any", {
      sequence: 20, callId, caseId, type: "CALL_ENDED", data: { status: "DISCONNECTED" }, timestamp: turn.timestamp,
    });
    await waitUntil(() => app.database.listSemanticCallEvents(callId).some((event) => event.eventType === "CALL_DISCONNECTED"));
    expect(app.database.getSupportThread(threadId)?.state).toBe("CALL_ENDING");

    const evidence = [{ turnId: turn.id, exactQuote: turn.text }];
    app.database.saveOutcome(callId, {
      status: "DISCONNECTED", summary: { value: turn.text, evidence }, representativeName: null,
      department: null, caseNumber: null, resolution: null, monetaryOutcomes: [], companyCommitments: [],
      userActions: [], deadlines: [], unresolvedItems: [{ value: turn.text, evidence }],
      endedAt: turn.timestamp, durationSeconds: 10, estimatedTelephonyCostUsd: 0,
    });
    app.service.events.emit("call:any", {
      sequence: 21, callId, caseId, type: "OUTCOME_GENERATED", data: { grounded: true }, timestamp: turn.timestamp,
    });
    await waitUntil(() => app.database.getSupportThread(threadId)?.state === "FAILED");
    expect(app.database.listSemanticCallEvents(callId).filter((event) => event.eventType === "CALL_DISCONNECTED")).toHaveLength(1);
  });
  ...
  it("rolls back commitments, confirmation events, completion outbox, and thread completion together", async () => {
    ...
    app.service.events.emit("call:any", {
      sequence: 30, callId, caseId, type: "OUTCOME_GENERATED", data: { grounded: true }, timestamp: turn.timestamp,
    });

    await waitUntil(() => app.database.listEvents(callId).some((event) => event.type === "TECHNICAL_ERROR" && JSON.stringify(event.payload).includes("MESSAGING_EVENT_PROJECTION_FAILED")));
    expect(app.database.listCommitments({ callId })).toHaveLength(0);
    expect(app.database.listSemanticCallEvents(callId).filter((event) => event.eventType === "COMMITMENT_CONFIRMED")).toHaveLength(0);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM outbound_messages WHERE idempotency_key=?").get(`outcome:${callId}`) as { count: number }).count).toBe(0);
