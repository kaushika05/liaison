...
const completeIssue = "Call Xfinity at +18009346489. They charged me a $35 installation fee even though installation was promised free. I want the fee removed. Account credit is acceptable only if it is at least $35. Do not change my plan or accept any new charge.";
...
function callCode(snapshot: MessagingThreadSnapshot): string {
  const match = [...snapshot.messages].reverse().find((message) => message.direction === "OUTBOUND")?.redactedBody.match(/CALL ([23456789A-HJ-NP-Z]{6})/);
...
describe("SMS-first messaging workflow", () => {
  it("runs one issue through plan, exact call authorization, both attention tiers, and grounded commitments", async () => {
    ...
    expect(snapshot.conditionalAuthorityRules).toEqual(expect.arrayContaining([
    ...
    const rawDatabaseAfterAuthorization = JSON.stringify(app.database.db.prepare("SELECT * FROM outbound_messages").all());
    ...
    const secureMessage = [...snapshot.messages].reverse().find((message) => message.direction === "OUTBOUND" && message.redactedBody.includes("/a/"));
    ...
    await expect(app.messaging.resolveSecureAction(token!, { decision: "REJECT" })).rejects.toThrow("ACTION_LINK_INVALID_OR_EXPIRED");
    snapshot = await waitFor(app, (value) => value.attention?.tier === "MATERIAL");
    const materialToken = [...snapshot.messages].reverse().find((message) => message.direction === "OUTBOUND" && message.redactedBody.includes("/a/"))?.redactedBody.match(/\/a\/([A-Za-z0-9_-]{43})/)?.[1];
    expect(materialToken).toBeTruthy();
    expect(app.messaging.secureAction(materialToken!).approvalPermitted).toBe(false);
    await expect(app.messaging.resolveSecureAction(materialToken!, { decision: "APPROVE", confirmation: "CONFIRM" })).rejects.toThrow("CONDITIONAL_AUTHORITY_DENIED");
    expect(app.messaging.secureAction(materialToken!).tokenState).toBe("VALID");
    expect((app.database.db.prepare("SELECT used_at FROM secure_action_tokens WHERE attention_request_id=?").get(snapshot.attention!.id) as { used_at: string | null }).used_at).toBeNull();
    ...
    await expect(app.messaging.resolveSecureAction(materialToken!, { decision: "REJECT" })).rejects.toThrow("ACTION_LINK_INVALID_OR_EXPIRED");
    ...
    expect(snapshot.messages.some((message) => message.direction === "OUTBOUND" && message.redactedBody.includes("Result:"))).toBe(true);

    const terminalCall = app.database.db.prepare("SELECT id,case_id FROM calls ORDER BY started_at DESC LIMIT 1").get() as { id: string; case_id: string };
    const commitmentCountBefore = (app.database.db.prepare("SELECT COUNT(*) AS count FROM commitments WHERE call_id=?").get(terminalCall.id) as { count: number }).count;
    const finalMessageCountBefore = (app.database.db.prepare("SELECT COUNT(*) AS count FROM outbound_messages WHERE idempotency_key=?").get(`outcome:${terminalCall.id}`) as { count: number }).count;
    app.database.updateSupportThread(snapshot.thread.id, { state: "CALL_ENDING", activeCallId: terminalCall.id });
    app.service.events.emit("call:any", { sequence: 99_999, callId: terminalCall.id, caseId: terminalCall.case_id, type: "OUTCOME_GENERATED", data: { replayed: true }, timestamp: new Date().toISOString() });
    await waitFor(app, (value) => value.thread.state === "COMPLETED" && value.thread.activeCallId === null);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM commitments WHERE call_id=?").get(terminalCall.id) as { count: number }).count).toBe(commitmentCountBefore);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM outbound_messages WHERE idempotency_key=?").get(`outcome:${terminalCall.id}`) as { count: number }).count).toBe(finalMessageCountBefore);
  ...
  it("asks for a missing destination and never invents or dials one", async () => {
    const app = await createContext();
    const snapshot = await app.messaging.receiveWebText("Company: Northstar. My order arrived broken. I want a no-cost replacement.");
    expect(snapshot.thread.state).toBe("AWAITING_INFORMATION");
  ...
  it("redacts prohibited credentials before persistence and does not invoke planning", async () => {
    ...
    const inbound = app.database.db.prepare("SELECT redacted_body,error_code FROM inbound_messages LIMIT 1").get() as { redacted_body: string; error_code: string };
  ...
  it("resumes a checkpointed inbound plan after a crash without duplicating its case or plan version", async () => {
    ...
    expect((app.database.db.prepare("SELECT json_extract(brief_json,'$.version') AS version FROM cases").get() as { version: number }).version).toBe(1);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM outbound_messages WHERE redacted_body LIKE 'PLAN %'").get() as { count: number }).count).toBe(0);

    planning.mockRestore();
    app.database.db.prepare("UPDATE messaging_work_items SET next_eligible_at=? WHERE state='PENDING'").run(new Date(0).toISOString());
    await app.messaging.flush();
    ...
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM cases").get() as { count: number }).count).toBe(1);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM outbound_messages WHERE redacted_body LIKE 'PLAN %'").get() as { count: number }).count).toBe(1);
    expect((app.database.db.prepare("SELECT state FROM messaging_work_items WHERE inbound_message_id=(SELECT id FROM inbound_messages WHERE redacted_body='YES')").get() as { state: string }).state).toBe("COMPLETED");
  });
  ...
  it("reuses the deterministic case binding when a crash lands after case insertion", async () => {
    ...
    await expect(app.messaging.receiveWebText("YES")).rejects.toThrow("INBOUND_PROCESSING_DID_NOT_SETTLE");
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM cases").get() as { count: number }).count).toBe(1);
    expect((app.database.db.prepare("SELECT brief_json FROM cases").get() as { brief_json: string | null }).brief_json).toBeNull();

    creation.mockRestore();
    app.database.db.prepare("UPDATE messaging_work_items SET next_eligible_at=? WHERE state='PENDING'").run(new Date(0).toISOString());
    const recovered = await waitFor(app,(value)=>value.thread.state==="AWAITING_PLAN_APPROVAL"&&value.case?.brief?.version===1);
    expect(recovered.thread.state).toBe("AWAITING_PLAN_APPROVAL");
    expect(recovered.case?.brief?.version).toBe(1);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM cases").get() as { count: number }).count).toBe(1);
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM events WHERE type='CASE_CREATED'").get() as { count: number }).count).toBe(1);
  });
  ...
  it("revises the current plan for ordinary added context and revokes an issued call authorization", async () => {
    ...
    snapshot = await app.messaging.receiveWebText("Also ask the representative to confirm in writing that this fee will not recur.");
    expect(snapshot.thread.state).toBe("AWAITING_PLAN_APPROVAL");
    ...
    expect(snapshot.thread.state).toBe("AWAITING_AVAILABILITY");
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM call_authorizations WHERE revoked_at IS NULL").get() as { count: number }).count).toBe(1);
    ...
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM call_authorizations WHERE revoked_at IS NULL").get() as { count: number }).count).toBe(0);
    expect((app.database.db.prepare("SELECT revoke_reason FROM call_authorizations ORDER BY created_at DESC LIMIT 1").get() as { revoke_reason: string }).revoke_reason).toBe("PLAN_REVISED_BY_MESSAGE");

    const denied = await app.messaging.receiveWebText(`CALL ${staleCode}`);
    expect([...denied.messages].reverse().find((message)=>message.direction==="OUTBOUND")?.redactedBody).toContain("invalid or stale");
    expect((app.database.db.prepare("SELECT COUNT(*) AS count FROM calls").get() as { count: number }).count).toBe(0);
  });

  it.each(["COMPLETED", "FAILED", "CANCELLED"] as const)("completes ordinary free text in terminal %s state with deterministic NEW guidance", async (state) => {
    const app = await createContext();
    ...
    expect((app.database.db.prepare("SELECT processing_state FROM inbound_messages ORDER BY created_at DESC LIMIT 1").get() as { processing_state: string }).processing_state).toBe("COMPLETED");
  });
});
