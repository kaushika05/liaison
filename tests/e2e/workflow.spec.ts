...
test("complete primary messaging workflow with low-risk and secure decisions",async({page})=>{
  ...
  const authorizationMessage=history.locator(".thread-message").filter({hasText:/reply exactly CALL [A-Z0-9]{4,8}/i}).last();
  ...
  const secureReview=page.locator(".secure-review").filter({has:page.getByRole("heading",{name:"Review this request"})});
  ...
  const commitments=page.locator(".message-side-panel").filter({has:page.getByRole("heading",{name:"Commitments"})});
  ...
  await expect(commitments.locator("blockquote").filter({hasText:/I approved a full \$35 account credit/i})).toBeVisible();
