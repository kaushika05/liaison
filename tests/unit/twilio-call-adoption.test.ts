...
function seedCall(id:string,state:"DIALING"|"ENDING",terminalReason:string|null):void{
  database!.createCase({id:`case-${id}`,companyName:"Example",title:"Support request",intake:{},disclosureMetadata:[]});
  database!.createCall({id,caseId:`case-${id}`,mode:"SIMULATOR",scenarioId:null,state,activity:"Calling",objective:"Resolve issue"});
