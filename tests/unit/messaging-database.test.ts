...
describe("durable messaging database", () => {
  it("migrates an existing database without losing old-schema rows", () => {
    ...
    old.exec(
      "CREATE TABLE cases (id TEXT PRIMARY KEY, company_name TEXT NOT NULL, title TEXT NOT NULL, status TEXT NOT NULL, intake_json TEXT NOT NULL, brief_json TEXT, disclosure_metadata_json TEXT NOT NULL DEFAULT '[]', approved_version INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)",
    );
    old.exec(
      "CREATE TABLE call_authorizations (id TEXT PRIMARY KEY, thread_id TEXT NOT NULL, case_id TEXT NOT NULL, plan_version INTEGER NOT NULL, code_hash TEXT NOT NULL, created_at TEXT NOT NULL, expires_at TEXT NOT NULL, consumed_at TEXT, revoked_at TEXT, revoke_reason TEXT)",
    );
    old
      .prepare("INSERT INTO cases VALUES (?,?,?,?,?,?,?,?,?,?)")
      .run("legacy", "Company", "Legacy", "DRAFT", "{}", null, "[]", null, T0, T0);
    old
      .prepare("INSERT INTO call_authorizations VALUES (?,?,?,?,?,?,?,?,?,?)")
    ...
    expect(
      (migrated.db.prepare("SELECT name FROM schema_migrations WHERE version=1").get() as { name: string }).name,
    ).toBe("sms_first_durable_messaging");
    expect(
      (migrated.db.prepare("SELECT name FROM schema_migrations WHERE version=2").get() as { name: string }).name,
    ).toBe("provider_security_and_authorization_binding");
    expect(
      migrated.db
        .prepare("SELECT destination_e164,telephony_mode FROM call_authorizations WHERE id='legacy-auth'")
        .get(),
    ...
    expect(
      (
        migrated.db
          .prepare(
            "SELECT COUNT(*) AS count FROM sqlite_master WHERE type='table' AND name IN ('support_threads','inbound_messages','outbound_messages','messaging_work_items')",
          )
          .get() as { count: number }
      ).count,
    ).toBe(4);
  ...
  it("deduplicates inbound provider messages and creates exactly one work item", () => {
    ...
    seed(db);
    const first = db.insertInboundMessageAndSchedule(inbound(), {
      id: "work-1",
      kind: "PROCESS_INBOUND",
      idempotencyKey: "work:SM123",
    });
    const duplicate = db.insertInboundMessageAndSchedule(inbound({ id: "in-duplicate" }), {
      id: "work-duplicate",
      kind: "PROCESS_INBOUND",
      idempotencyKey: "work:SM123",
    });
    ...
    expect((db.db.prepare("SELECT COUNT(*) AS count FROM messaging_work_items").get() as { count: number }).count).toBe(
      1,
    );
    db.close();
  });
  ...
  it("persists pending work across close and reopen", () => {
    ...
    seed(before);
    before.insertInboundMessageAndSchedule(inbound(), {
      id: "work-1",
      kind: "PROCESS_INBOUND",
      idempotencyKey: "work:SM123",
    });
  ...
  it("deduplicates the outbox and dead-letters an ambiguous expired send lease without retry", () => {
    ...
    expect(db.markOutboundSent({ id: "out-1", workerId: "worker-a", providerMessageId: "SM-out", now: T61 })).toBe(
      false,
    );
    expect(db.markOutboundSent({ id: "out-1", workerId: "worker-b", providerMessageId: "SM-out", now: T61 })).toBe(
      false,
    );
    expect(db.getOutboundMessage("out-1")).toMatchObject({
  ...
  it("atomically opts out a thread and cancels every locally queued outbound send", () => {
    ...
    db.enqueueOutboundMessage(
      outbound({ id: "d-web", idempotencyKey: "web", providerKind: "WEB", recipient: "WEB_OWNER" }),
    );
    expect(db.claimOutboundMessages({ workerId: "sender", now: T0, leaseSeconds: 60, limit: 1 })[0]?.id).toBe(
      "b-processing",
    );

    expect(db.optOutSupportThreadAndCancelOutbound(threadId, { now: T30 })).toEqual({
      threadUpdated: true,
      cancelledCount: 2,
    });
    expect(db.getSupportThread(threadId)?.messagingOptState).toBe("OPTED_OUT");
    expect(db.getOutboundMessage("b-processing")).toMatchObject({
      processingState: "DEAD_LETTER",
      deliveryState: "FAILED",
      errorCode: "OWNER_OPTED_OUT",
      leaseOwner: null,
    });
    expect(db.getOutboundMessage("c-pending")).toMatchObject({
      processingState: "DEAD_LETTER",
      deliveryState: "FAILED",
      errorCode: "OWNER_OPTED_OUT",
    });
    expect(db.getOutboundMessage("a-sent")).toMatchObject({
      processingState: "COMPLETED",
      deliveryState: "QUEUED",
      providerMessageId: "SM-sent",
    });
    expect(db.getOutboundMessage("d-web")).toMatchObject({
      processingState: "PENDING",
      deliveryState: "PENDING",
      errorCode: null,
    });
    expect(
      db.claimOutboundMessages({ workerId: "sender", now: T61, leaseSeconds: 60 }).map((message) => message.id),
    ).toEqual(["d-web"]);
    db.close();
  });
  ...
  it("consumes call authorization only once for the exact current plan before expiry", () => {
    ...
    db.createCallAuthorization({
      id: "auth-1",
      threadId,
      caseId,
      planVersion: 1,
      destinationE164: "+13045550123",
    ...
    expect(
      db.consumeCallAuthorization({
        threadId,
        caseId,
        planVersion: 1,
        destinationE164: "+13045550999",
        telephonyMode: "simulator",
        codeHash: HASH_A,
        now: T30,
      }),
    ...
    expect(
      (
        db.db.prepare("SELECT revoke_reason FROM call_authorizations WHERE id='auth-invalidated'").get() as {
  ...
  it("enforces secure-token expiry, revocation, exact binding, and single use", () => {
    const db = new LiaisonDatabase(":memory:");
    const { caseId, threadId } = seed(db);
    const create = (id: string, hash: string, expiresAt: string) =>
      db.createSecureActionToken({
        id,
        tokenHash: hash,
    ...
    expect(
      db.consumeSecureActionToken({ tokenHash: HASH_A, actionType: "WRONG", threadId, caseId, now: T30 }),
    ).toBeNull();
    expect(
      db.consumeSecureActionToken({ tokenHash: HASH_A, actionType: "APPROVE_PLAN", threadId, caseId, now: T30 })?.id,
    ).toBe("token-1");
    expect(
      db.consumeSecureActionToken({ tokenHash: HASH_A, actionType: "APPROVE_PLAN", threadId, caseId, now: T30 }),
    ).toBeNull();

    create("token-expired", HASH_B, T30);
    expect(
      db.consumeSecureActionToken({ tokenHash: HASH_B, actionType: "APPROVE_PLAN", threadId, caseId, now: T30 }),
    ).toBeNull();
    create("token-revoked", HASH_C, T61);
    expect(db.revokeSecureActionTokens({ tokenHash: HASH_C, reason: "TEST", now: T30 })).toBe(1);
    expect(
      db.consumeSecureActionToken({ tokenHash: HASH_C, actionType: "APPROVE_PLAN", threadId, caseId, now: T30 }),
    ).toBeNull();
    db.close();
  });
  ...
  it("allows only one blocking attention request and rejects stale resolution", () => {
    ...
    db.createSecureActionToken({
      ...
      caseId,
      callId,
      attentionRequestId: first.request.id,
      expiresAt: T61,
      now: T0,
    });
    ...
    expect(
      db.resolveAttentionRequest({ id: "attention-1", expectedCallId: callId, resolution: { choice: "A" }, now: T30 }),
    ).toBeNull();
    expect(
      db.resolveAttentionRequest({
        id: "attention-2",
        expectedCallId: callId,
        resolution: { reviewed: true },
        now: T30,
      })?.status,
    ).toBe("RESOLVED");
    expect(
  ...
  it("requires commitment evidence and deduplicates semantic call events", () => {
    ...
    const base = {
      ...
      description: "Company will issue a $35 credit.",
      amountCents: 3500,
      now: T0,
    };
    expect(() => db.createCommitment({ ...base, evidence: [] })).toThrow("COMMITMENT_EVIDENCE_REQUIRED");
    const stored = db.createCommitment({
      ...base,
      evidence: [{ turnId: "turn-4", exactQuote: "I can issue a $35 credit." }],
    });
    ...
    const event = {
      ...
      semanticKey: "credit-35",
      payload: { commitmentId: stored.id },
      occurredAt: T0,
    };
  ...
  it("deletes correlated records without orphaning legacy events and retains nonterminal cases", () => {
    ...
    db.insertInboundMessageAndSchedule(
      inbound({
        id: "in-pre-case",
    ...
    db.db
      ...
      .run("delivery-1", "out-case", "SM-out-case", "delivered", null, T0, T0, "delivery:private-case-copy");
    db.appendEvent({
      id: "event-1",
      callId: callId as string,
      caseId,
    ...
    db.insertInboundMessageAndSchedule(
      inbound({ id: "in-case", providerMessageId: "SM-case", caseId, callId, idempotencyKey: "twilio:SM-case" }),
      { id: "work-1", kind: "PROCESS_INBOUND", idempotencyKey: "work:SM-case" },
    );
    const attention = db.createAttentionRequest({
      id: "attention-delete",
      threadId,
      caseId,
      callId,
      tier: "SENSITIVE",
      question: "private decision copy",
      expiresAt: T61,
      now: T0,
    }).request;
    db.createSecureActionToken({
    ...
    db.createConditionalAuthorityRule({
      id: "rule-delete",
      threadId,
      caseId,
      actionType: "private action copy",
      condition: { note: "private condition copy" },
      permission: "ASK",
      now: T0,
    });
    db.createCommitment({
      ...
      party: "COMPANY",
      status: "CONFIRMED",
      description: "Replacement ships",
      evidence: [{ turnId: "turn-1", exactQuote: "We will ship it." }],
      now: T0,
    });
    db.insertSemanticCallEvent({
      id: "semantic-delete",
      threadId,
      caseId,
      callId: callId as string,
      eventType: "PRIVATE_EVENT",
      semanticKey: "private-event",
      payload: { note: "private semantic copy" },
      occurredAt: T0,
    });
    db.recordProviderSecurityEvent({
      id: "security-delete",
      providerKind: "TWILIO_SMS",
      providerMessageId: "SM-security-delete",
      eventType: "CALLBACK_REJECTED",
      reasonCode: "TEST",
      threadId,
      caseId,
      callId,
      redactedMetadata: { note: "private provider copy" },
      createdAt: T0,
    });
    db.addTranscript(callId as string, {
      id: "turn-1",
      sequence: 1,
      speaker: "REMOTE",
      text: "private transcript copy",
      timestamp: T0,
    });
    db.db
    ...
    db.db
      .prepare("INSERT INTO outcome_reports (call_id,report_json,created_at) VALUES (?,?,?)")
    ...
    const erasedTables = [
      ...
      "messaging_work_items",
      "attention_requests",
      "secure_action_tokens",
      "conditional_authority_rules",
      "commitments",
      "semantic_call_events",
      "call_authorizations",
      "provider_security_events",
    ];
    ...
    expect(JSON.stringify(db.db.prepare("SELECT * FROM support_threads").all())).not.toContain(
      "private pre-case draft copy",
    );
    expect(JSON.stringify(db.db.prepare("SELECT * FROM inbound_messages").all())).not.toContain(
      "private pre-case inbound copy",
    );
    expect(JSON.stringify(db.db.prepare("SELECT * FROM outbound_messages").all())).not.toContain(
      "private pre-case outbound copy",
    );
