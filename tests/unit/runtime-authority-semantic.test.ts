...
} from "../../src/server/core/runtime-authority.js";
...
} from "../../src/server/messaging/semantic-extraction.js";
...
const approval = (overrides: Partial<ApprovalRequest> = {}): ApprovalRequest => ({
  ...
  representativeRequest: "Upgrade the plan for six months?",
  proposedSpeech: "The account holder approves the plan upgrade.",
  consequences: "This changes the service plan.",
...
describe("runtime conditional authority", () => {
  ...
  it("uses only rules bound to the approved version and never downgrades the material tier", () => {
    ...
    expect(evaluateApprovalConditionalAuthority(approval(), rules.map(record), 3)).toMatchObject({
      decision: "DENY",
      subject: "PLAN_CHANGE",
    });
    expect(evaluateApprovalConditionalAuthority(approval(), rules.map(record), 4)).toMatchObject({
      decision: null,
      source: "NO_MATCH",
    });
...
describe("semantic extraction", () => {
  const turn: TranscriptTurn = {
    ...
    text: "I approved a full $35 account credit. It will appear on the next billing statement. Case number B-19382.",
  };
    ...
    expect(events).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "CASE_NUMBER_RECEIVED",
          evidence: [{ turnId: "turn-7", exactQuote: "Case number B-19382" }],
        }),
  ...
  it("does not turn refusals into positive transfer or offer updates", () => {
    for (const text of [
      ...
      "We do not offer account credits.",
      "I cannot transfer you to billing.",
    ]) {
      const events = semanticEventsFromFinalTurn({ ...turn, text }).map((item) => item.event.kind);
      expect(events).not.toContain("OFFER_MADE");
      expect(events).not.toContain("TRANSFER_STARTED");
    }
  });
  ...
  it("accepts structured facts only when a final remote turn contains the exact value", () => {
    ...
    expect(events).toEqual(
      expect.arrayContaining([
        ...
        {
          kind: "FACT_CONFIRMED",
          fact: "will issue a replacement",
          evidence: [{ turnId: remote.id, exactQuote: "will issue a replacement" }],
        },
  ...
  it("projects verified resolution fields without inventing commitments", () => {
    ...
    const report = {
      status: "RESOLVED",
      summary: { value: "Resolved", evidence },
      representativeName: null,
      department: null,
      caseNumber: { value: "B-19382", evidence },
      resolution: { value: "A credit was applied", evidence },
      monetaryOutcomes: [],
      companyCommitments: [],
      userActions: [],
      deadlines: [],
    ...
    expect(semanticEventsFromOutcome(report).map((item) => item.event.kind)).toEqual([
  ...
  it("reports a grounded disconnect outcome as a semantic disconnect", () => {
    ...
    const report = {
      status: "DISCONNECTED",
      summary: { value: "The remote call disconnected", evidence },
      representativeName: null,
      department: null,
      caseNumber: null,
      resolution: null,
      ...
      userActions: [],
      deadlines: [],
      unresolvedItems: [{ value: "The support request remains unresolved", evidence }],
      endedAt: turn.timestamp,
      durationSeconds: 10,
      estimatedTelephonyCostUsd: 0,
    } satisfies OutcomeReport;
