  ...
  it("creates and terminates a real outbound Calls API resource through the SDK boundary", async () => {
    ...
    expect(create).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        from: "+12125550199",
        record: false,
        url: "https://liaison.example/webhooks/twilio/voice/opaque.token",
        statusCallbackEvent: ["initiated", "ringing", "answered", "completed"],
      }),
    );
    await adapter.endCall("call-1", "USER_REQUESTED");
    expect(callResource).toHaveBeenCalledWith("CA123");
    expect(update).toHaveBeenCalledWith({ status: "completed" });
    await adapter.endOrphanedCall("CA-orphan");
    expect(callResource).toHaveBeenCalledWith("CA-orphan");
  });
});
