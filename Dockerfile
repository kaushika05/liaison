...
RUN apt-get update \
    ...
    && groupadd --system liaison \
    && useradd --system --gid liaison --home-dir /app liaison \
    && mkdir -p /data \
    && chown liaison:liaison /data
...
CMD ["sh","-c","chown -R liaison:liaison /data && exec gosu liaison node dist-server/server/index.js"]
